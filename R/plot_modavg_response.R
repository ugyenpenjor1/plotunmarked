...
plot_modavg_response <- function(
    ...
    ci_level = 0.95           # confidence interval level (e.g., 0.85, 0.9, 0.95)
) {
  ...
  if (!("model" %in% colnames(model_selection_table))) stop("model_selection_table must contain a 'model' column")
  ...
  # Predict with specified CI level
  ...
  pred_df[[covariate]] <- newdata[[covariate]]  # attach covariate for plotting
  ...
  p <- ggplot2::ggplot(pred_df) +
    ...
    ggplot2::labs(
      x = xlab,
      y = ifelse(response_type == "state", "Probability of occupancy", "Probability of detection")
    ) +
    ggplot2::ylim(0, 1) +
